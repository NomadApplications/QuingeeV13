    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        if(interaction.commandName === 'home'){
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setDescription(`<@${interaction.user.id}> - **House Type**: *${profile.houseType.name}*`)
            ...
            let item = profile.nodeSlots.find(x=>{
                if(x !== null && x.name===interaction.options.getString('item')) return true;
            });
            if (item === undefined) {
            ...
            const cId = interaction.user.id;

            client.once('interactionCreate', async (interaction) => {
                if(!interaction.isButton()) return;

                if(interaction.customId === 'confirm' && interaction.user.id == cId){
