...
function handleCommands(client) {
    client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.commandName === "mine") {
            await mineGame(interaction);
        } else if (interaction.commandName === 'fish') {
            await fishGame(interaction);
        } else if (interaction.commandName === 'gather') {
            await gatherGame(interaction);
        }
    })
}

async function mineGame(interaction){
    let found = false;
    let seconds = 0;
    ...
    client.once('interactionCreate', async (interaction) => {
        ...
        if (id.includes('mine')) {
            ...
            if(item === null || item === undefined) {
                replyError(interaction, "There are no items currently available!");
                return;
            }
            ...
            await interaction.update({
                embeds: [new Discord.MessageEmbed()
                    ...
                    .setDescription(`${interaction.user} ${m} With a whole lot of effort on your part, you are able to unearth one **${capitalize(item.name)} ${em}**! Your new item has been added to *${profile.title}.*`)],
                components: []
            });
...
async function fishGame(interaction){
    let found = false;
    let seconds = 0;
            ...
            client.once('interactionCreate', async (interaction) => {
                ...
                if (interaction.customId === 'fish' + interaction.user.id) {
                    ...
                    const e = new Discord.MessageEmbed()
                        ...
                        .setColor(config.colors.successColor)
                        .setDescription(`${interaction.user} ${m} With a big heave and a giant *whoa*, you reel back one **${capitalize(item.name)} ${em}**! Your catch has been added to *${profile.title}.*`);
...
async function gatherGame(interaction){
    let found = false;
    let seconds = 0;
        ...
        if (interaction.customId === 'gather' + interaction.user.id) {
            ...
            await interaction.reply({
                embeds: [new Discord.MessageEmbed()
                    ...
                    .setColor(config.colors.successColor)
                    .setDescription(`${interaction.user} ${m} Within a bush, betwixt some roots, high in the trees or nestled in the tall grass, you find one **${capitalize(item.name)} ${em}**! A nice reward for a hard day's work. Your forage has been added to *${profile.title}.*`)],
                components: []
            });
...
function getItemFromLevel(level, category) {
    ...
    if (level === 1) {
        if (category === "mining") {
            item = getRandom(minigamesConfig.mining.level1.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "fishing") {
            item = getRandom(minigamesConfig.fishing.level1.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "gathering") {
            item = getRandom(minigamesConfig.gathering.level1.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        }
    } else if (level === 2) {
        if (category === "mining") {
            item = getRandom(minigamesConfig.mining.level2.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "fishing") {
            item = getRandom(minigamesConfig.fishing.level2.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "gathering") {
            item = getRandom(minigamesConfig.gathering.level2.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        }
    } else if (level === 3) {
        if (category === "mining") {
            item = getRandom(minigamesConfig.mining.level3.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "fishing") {
            item = getRandom(minigamesConfig.fishing.level3.items.filter(x=>x.season===db.get("seasons.currentSeason")||x.season==="all"))
        } else if (category === "gathering") {
            item = getRandom(minigamesConfig.gathering.level3.items.filter(x=> x.season===db.get("seasons.currentSeason")||x.season==="all"))
        }
    }
    return item;
}