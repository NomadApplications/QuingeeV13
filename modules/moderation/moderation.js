module.exports = function (client) {
    client.on("messageCreate", message => {
        ...
        if (command === prefix + "ban") {
            ...
            for(let i = 0; i < amount; i++) addItemToProfile(profile, item);

            const embed = new Discord.MessageEmbed()
                .setColor(config.colors.successColor)
                .setDescription("You have successfully given " + mentionedUser.user.toString() + " x" + amount + " " + capitalize(item.name) + "!");

            embed.setFooter("Quingee Bot", message.author.displayAvatarURL());

            message.channel.send({embeds: [embed]});
        } else if (command === prefix + "take"){
            if(!message.member.permissionsIn(message.channel).has("ADMINISTRATOR")) return;
            if(args.length < 2) return rError(message, "``" + prefix + "give <user> <item> [amount]``");
            ...
            for(let i = 0; i < amount; i++) removeItem(profile, item);

            const embed = new Discord.MessageEmbed()
                .setColor(config.colors.successColor)
                .setDescription("You have successfully remove x" + amount + " " + capitalize(item.name) + " from " + mentionedUser.user.toString() + "!");
            ...
            if(!message.member.permissionsIn(message.channel).has("ADMINISTRATOR")) return;
            if(args.length < 1) return rError(message, "``" + prefix + "add <user> <amount>``");

            const userToGive = message.mentions.members.first();
            if(userToGive === null || userToGive === undefined) return rError(message, "Please specify a valid user.");

            if(args[1] === undefined) {
                return rError(message, "Please specify amount.");
            }
            if(isNaN(args[1])) return message.channel.send("Please specify valid number.");

            addCurrency(getProfileByString('Bank', userToGive), parseInt(args[1]));
            message.channel.send({
                embeds: [
                    new Discord.MessageEmbed()
                        .setColor(config.colors.defaultColor)
                        .setDescription(`Successfully added **${config.economy.moneyPrefix}${args[1]}** to ${userToGive.user}`)
                        .setFooter("Quingee Bot", message.author.displayAvatarURL())
                ]
            });
        } else if (command === prefix + 'remove'){
            if(!message.member.permissionsIn(message.channel).has("ADMINISTRATOR")) return;
            if(args.length < 1) return rError(message, "``" + prefix + "remove <user> <amount>``");
            ...
            removeCurrency(getProfileByString('Bank', userToGive), parseInt(args[1]));

            message.channel.send({
                embeds: [
                    new Discord.MessageEmbed()
                        .setColor(config.colors.defaultColor)
                        .setDescription(`Successfully removed **${config.economy.moneyPrefix}${args[1]}** from ${userToGive.user}`)
                        .setFooter("Quingee Bot", message.author.displayAvatarURL())
                ]
            });
        } else if (command === prefix + "newday"){
            if(!message.member.permissionsIn(message.channel).has("ADMINISTRATOR")) return;
            newDay();
        }
    });
